...
Prerequisites:
    ...
    apt-get install mysql-client mysql-server python-mysqldb python-crypto python-pip
...
Then run:
    ...
    python -m Abe.abe --config abe.conf --commit-bytes 200000 --no-serve
    
After the block chain has finished being uploaded to the database, then run:
