...
ABE_URL = 'https://github.com/geekz/bitcoin-abe'
...
<link href="%(dotdot)s%(STATIC_PATH)scss/jquery.dataTables.css" rel="stylesheet" type="text/css">
<!-- Custom styles for this template -->
<link href="%(dotdot)s%(STATIC_PATH)scss/style.css" rel="stylesheet" type="text/css">
<!-- HTML5 shim and Respond.js IE8 support of HTML5 elements and media queries -->
<!--[if lt IE 9]>
      <script src="%(dotdot)s%(STATIC_PATH)sjs/html5shiv.js"></script>
      <script src="%(dotdot)s%(STATIC_PATH)sjs/respond.min.js"></script>
    <![endif]-->
</head>
...
<!--/.container-->
<script src="%(dotdot)s%(STATIC_PATH)sjs/jquery-1.10.2.min.js"></script>
...
class Abe:
    ...
    def handle_chain(abe, page):
        ...
        """)
        for row in rows:
            tx_id, tx_hash_hex, tx_size = (row[0], abe.store.hashout_hex(row[1]),row[2])
            body+='<tr><td><a href="',basename,'/tx/', tx_hash_hex,'">', tx_hash_hex[:14],'</a></td><td class="hidden">', tx_hash_hex,'</td><td>', tx_size,'(bytes)</td></tr>';
    ...
    def handle_address(abe, page):
        ...
        body += ['<h1 style="padding-bottom:10px">BitQuark Address <small>Addresses are identifiers which you use to send BitQuark coins to another person.</small></h1>']
